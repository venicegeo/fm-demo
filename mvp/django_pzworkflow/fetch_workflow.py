...
class PzWorkflow:
    ...
    def get_all(self, user_request):
        # if user_request.get('type') == 'event':
        #     return requests.get(self.addr + self.map.get(user_request.get('type')) + 'fulcrum-map-event')
    ...
    def delete(self, user_request):
        ...
        if check:
            print "Deleting"
            if user_request.get('type') == "event":
                eventtype_request = {'type': 'eventtypes', 'data': {'id': check.get('eventtype_id')}}
                event_type = self.get(eventtype_request)
                if event_type:
                    deleted_id = requests.delete(self.addr + self.map.get(user_request.get('type'))  + event_type.get('name') + "/" + user_request.get('data').get('id'))
                else:
                    print "Could not get event-type name"
                    return None
            else:
